...
SELECT film.title, film.replacement_cost FROM rental
...
SELECT DISTINCT film.title FROM customer
...
SELECT DISTINCT film.title, film.replacement_cost FROM customer
JOIN rental ON customer.customer_id = rental.customer_id
JOIN inventory ON rental.inventory_id = inventory.inventory_id
JOIN film ON inventory.film_id = film.film_id
WHERE customer.first_name = 'Matthew' AND customer.last_name = 'Mahan'
